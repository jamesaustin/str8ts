...
class Board(dict):
    ...
    def sea_creatures_by_col(self):
        def _sea_creatures_by_col(d):
            ...
            for x in ACROSS:
                candidates = set(
                    [y for y in DOWN if self[x, y].is_unknown() and d in self[x, y].sure_candidates_by_col]
                )
                if candidates:
    ...
    def settis_rule(self):
        ...
        def _settis_rule(d):
            ...
            if False:
                print(
                    "d:{} row[y{}-m{}-n{}] col[y{}-m{}-n{}]".format(
                        d, len(row_yes), len(row_maybe), len(row_no), len(col_yes), len(col_maybe), len(col_no)
                    )
                )
                print("rY:", row_yes)
                print("rM:", row_maybe)
        ...
        for (x, y), c in self.items():
            if len(c.digits) == 2:
                ...
                for (dx, dy), dc in self._iter_all_but(x, y):
                    if (
                        dc.is_unknown()
                        and (len(self[dx, y].digits) == 2)
                        and (len(self[x, dy].digits) == 2)
                        and (
                            ((c0 in self[dx, y].digits) and (c1 in self[x, dy].digits))
                            or ((c1 in self[dx, y].digits) and (c0 in self[x, dy].digits))
                        )
                    ):
    ...
    def sure_candidate_upgrade_by_row(self):
        return any(
            sure_candidate_upgrade_by_cells(
                self.compartments_by_row[y], self.sure_candidates_by_cross_row[y], Cell.get_sc_by_row
            )
            for y in DOWN
        )
    ...
    def sure_candidate_upgrade_by_col(self):
        return any(
            sure_candidate_upgrade_by_cells(
                self.compartments_by_col[x], self.sure_candidates_by_cross_col[x], Cell.get_sc_by_col
            )
            for x in ACROSS
        )
...
def tests():
    ...
    test(
        [Board.sure_candidates_by_row, Board.sure_candidates_by_col],
        [
    ...
    # pg 9. Singles
    test([Board.singles_by_row, Board.singles_by_col], [["358", "357", "456", "3457"]], [["358", "357", "6", "3457"]])
    # pg 10. Compartment Range Checks
    test(
        [Board.compartment_range_check_by_row, Board.compartment_range_check_by_col],
    ...
    test(
        [Board.stranded_digits_by_row, Board.stranded_digits_by_col],
        [["1245679", "12679", "1245679", "c", "8"]],
        [["4567", "67", "4567", "#", "8"]],
    )
    # pg 13. Stranded Digits - Bridging Digits
    test(
        [Board.bridging_digits_by_row, Board.bridging_digits_by_col],
        [["13456", "1234568", "145678", "134568"]],
        [["13456", "234568", "14567", "134568"]],
    )
    test([Board.bridging_digits_by_row, Board.bridging_digits_by_col], [["13679", "1234689"]], [["1379", "2468"]])
    test([Board.bridging_digits_by_row, Board.bridging_digits_by_col], [["1", "3", "23"]], [["1", "3", "2"]])
    # bonus. Stranded Digits - Bridging Digits
    test([Board.bridging_digits_by_row, Board.bridging_digits_by_col], [["568", "5689", "7"]], [["568", "5689", "7"]])
    test([Board.bridging_digits_by_row, Board.bridging_digits_by_col], [["68", "7", "568"]], [["68", "7", "568"]])
    test(
        [Board.bridging_digits_by_row, Board.bridging_digits_by_col],
    ...
    test(
        [Board.stranded_by_bridge_by_row, Board.stranded_by_bridge_by_col],
        [["347", "2347", "34568", "1245"]],
        [["347", "2347", "3456", "1245"]],
    )
    test(
    ...
    test(
        [Board.split_compartments_by_row, Board.split_compartments_by_col],
    ...
    # pg 16. Mind the Gap
    test(
        [Board.mind_the_gap_by_row, Board.mind_the_gap_by_col],
        [["123456", "345678", "27", "123457"]],
        [["13456", "34568", "27", "1345"]],
    )
    # pg 17. Mind the Gap
    test(
        [Board.mind_the_gap_by_row, Board.mind_the_gap_by_col],
        [["123456", "345678", "237", "123457"], ["123456", "345678", "2378", "123457"]],
        [["123456", "34568", "237", "12345"], ["123456", "345678", "2378", "123457"]],
    )
    # pg 18. Mind the Gap
    test(
        [Board.mind_the_bridging_gap_by_row, Board.mind_the_bridging_gap_by_col],
        [["35", "2345678", "58", "123457", "13456"]],
        [["35", "234678", "58", "12347", "1346"]],
    )
    # pg 19. Naked Pair
    test(
        [Board.naked_groups_by_row, Board.naked_groups_by_col],
    ...
    # pg 22. Hidden Pair
    test(
        [Board.hidden_group_by_row, Board.hidden_group_by_col],
    ...
    test(
        [Board.hidden_group_cross_by_row, Board.hidden_group_cross_by_col],
    ...
    # pg 28, 29. X-Wing
    test(
        [Board.sea_creatures_by_col, Board.sea_creatures_by_row],
    ...
    # pg 30. X-Wing
    test(
        [Board.sea_creatures_by_row, Board.sea_creatures_by_col],
        [
    ...
    test(
        [Board.sea_creatures_cross_by_col, Board.sea_creatures_cross_by_row],
        # col and row deliberately swapped to fit test case
        [
            ["134", "1345", "1356789"],
            ["1234", "12345", "1234568"],
    ...
    test(
        Board.unique_rectangle_rule,
        [["1236789", "45", "45", "1236789"], ["1236789", "456", "45", "1236789"], ["123456", "1236", "1236", "123456"]],
        [["1236789", "45", "45", "1236789"], ["1236789", "6", "45", "1236789"], ["123456", "1236", "1236", "123456"]],
    )
    test(
        Board.unique_rectangle_rule,
        [
    ...
    test(
        [Board.split_compartments_by_col, Board.split_compartments_by_row],
        # col and row deliberately swapped to fit test case
        [
            ["134689", "#", "#", "3468"],
            ["2349", "6", "#", "7"],
    ...
    # bonus
    test(
        Board.unique_solution_constraint,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--chain-length", type=int, default=4, help="maximum length of cell to search for contradictions"
    )
    ...
    parser.add_argument("--hint", metavar="RC=N", nargs="+", help="Manual include a hint whilst solving, example A1=1")
    ...
    def _log_exception(name, e, b):
        _critical('Invalid board for "{0}", {1[0]}{1[1]} {2} is {1[2]}'.format(name, e, "".join(b[e[0], e[1]].digits)))
        print(b._to_string())
    ...
    if args.counts:
        ...
        for rule, hit in sorted(hits.items(), reverse=True, key=lambda k_v: k_v[1]):
            _hit(
                " {:<30} [#{:2}] [{:>5.1%} {:>7.2f}s] [{:>5.1%} {:>6}] [{:>5.1%} {:>10}]".format(
                    rule,
                    rules.index(rule),
                    durations[rule] / duration_total,
                    durations[rule],
                    float(hit) / hits_total,
                    hit,
                    float(counts[rule]) / counts_total,
                    counts[rule],
                )
            )
        for rule_fn in Board.steps:
            ...
            if rule not in hits:
                _miss(
                    "{:<30} [#{:2}] [{:>5.1%} {:>7.2f}s]".format(
                        rule, rules.index(rule), durations[rule] / duration_total, durations[rule]
                    )
                )
    if args.times:
        duration_total = sum(durations.values())
